...
A. Alter the code to select just the `first_name` column. Assign the result to `covid_testing_2` and show the resulting data frame by executing the following line with just the name of the object: `covid_testing_2` 
...
In the `covid_testing` dataset the turnaround time metrics are found in columns 16 and 17 and are labeled `col_rec_tat` and `rec_var_tat`. Your goal is to you selectively remove these columns. Using what we learned as well as the dplyr cheatsheet which of the following code snippets could you use to remove these columns?
...
CHALLENGE: Use your mouse to select the name of the new data frame from the list of objects in the Environment tab in the upper-right pane of Rstudio. 
...
covid_testing %>% 
  ...
  arrange(pan_day)    #Here arrange the pan_day column from highest to lowest (check slide 42 the arrange section of this file to use the desc() function)
```
...
```{r Your Turn 6}
covid_testing %>% 
  mutate(totals = col_rec_tat+rec_ver_tat)
```
