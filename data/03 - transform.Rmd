...
```{r}
filter(orders, `Patient ID` == 505646)
```
...
Alter the 3 code blocks below and use filter() to show:
...
```{r}
filter(orders, name == "Sea" | name == "Anemone")
```
...
Arrange orders by `n`. Add `prop` as a second (tie breaking) variable to arrange on. Can you tell what the smallest value of `n` is?
