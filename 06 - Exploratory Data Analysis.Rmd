...
- Use the fill aesthetic to parse out the contribution of different **category** or **fluid** values to the distribution of laboratory values

```{r}
mimic %>% filter(category=="Chemistry"|category=="Hematology") %>% 
ggplot()+
...
mimic %>% 
  filter(panel_test == 'CBC', component=='Hemoglobin') %>% 
  ggplot()+
  geom_histogram(aes(valuenum, fill=flag),binwidth = .1)

mimic %>% filter(panel_test == 'CBC', component=='Hemoglobin',flag=='abnormal',valuenum>12.5) %>% View
```

1. What do you notice about the pattern of **charttime**s and **valuenum**s in the data frame?
2. Are there differences between the distribution of "Chemistry" and "Hematology" test **category**s?
...
We have a very basic sense for what the data in mimic looks like. Now let's see if we can answer some questions using this data. For the purposes of this exercise we'll keep the clinical aspects of the question simple. Do patients with sepsis have higher white blood cell counts than those without sepsis?

- Examine the code below and execute the code block to create the mimic_sepsis data frame
- Use a statistical test to see whether the average maximum WBC count is different amongst those with and without sepsis.
- Make a plot which compares the maximum WBC counts for admissions with "SEPSIS" as the **diagnosis**. 

```{r}
mimic_sepsis<-mimic %>% 
  filter(component == 'White Blood Cells',
         !is.na(diagnosis)) %>% 
  mutate(sepsis = ifelse(str_detect(diagnosis,"SEPSIS"),"septic","not septic")) %>%
  group_by(hadm_id,sepsis) %>% 
  summarize(hadm_max_wbc = max(valuenum, na.rm = T))

t.test(mimic_sepsis$hadm_max_wbc~mimic_sepsis$sepsis)

mimic_sepsis  %>% 
  ggplot(aes(x=as.character(sepsis),y=hadm_max_wbc))+
  geom_boxplot()
  ylim(0,30)

```

1. Step through the analysis pipe line, what does each step do?
2. Do maximum WBC counts differ amongst these two groups? Is this a clinically useful test for sepsis detection?
3. *Challenge Question*: There seems to be several outliers amongst the "not septic" group. Examine what could be causing this. Does the model need to be refined?
...
mimic_dets<- mimic %>% 
  ...
  summarize(admit = min(charttime),
            discharge = max(charttime),
            panels = n_distinct(charttime,test_name) ) %>% 
  mutate(LOS = as.numeric(difftime(discharge,admit,units = "days")))

mimic_dets<-mimic_dets %>% filter(!is.infinite(loglos))# mutate(loglos = log10(LOS),logpan = log10(panels))
lm(loglos~logpan,data = mimic_dets) %>% summary
```


```{r}
#%>% 
  ggplot(aes(LOS,panels))+
  geom_point()+
  scale_x_log10()+
  scale_y_log10()+
  geom_smooth(method = "lm")

mimic %>% filter(hadm_id==142345) %>% View
```




LOOK AT LOS USING MIN MAX
LOOK AT TWO CBC IN A DAY
lfts



```{r}
mimic %>% filter(category %in% c("HEMATOLOGY","CHEMISTRY"))
```
