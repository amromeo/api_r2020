...
```{r}
mimic %>% group_by(subject_id) %>%  summarise(hadms = n_distinct(hadm_id)) %>% summarise(max(hadms))
  group_by(panel_test) %>% 
  summarise(n=n_distinct(component)) %>% arrange(desc(n))
```
