...
menu <- read_csv("data/test_catalog.csv")
```
...
1. Plot the distribution of orders by week instead of by day.
2. Plot the distribution of review times by day.
...
```{r}
ggplot(orders) + 
  geom_histogram(aes(x = order_time), binwidth = 7*24*60*60)
```

```{r}
ggplot(orders) + 
  geom_histogram(aes(x = review_time), binwidth = 24*60*60)
```
